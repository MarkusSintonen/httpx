...
REQUESTS = 500
CONCURRENCY = 20
POOL_LIMIT = httpx.Limits(max_connections=100, max_keepalive_connections=100)
PROFILE = False
...
async def run_async_requests(axis: Axes) -> None:
    ...
    async def httpx_get(client: httpx.AsyncClient, timings: List[int]) -> None:
    ...
    transport = httpx.AsyncHTTPTransport(limits=POOL_LIMIT, backend=NET_BACKEND())
    async with httpx.AsyncClient(transport=transport) as pool:
        # warmup
        await gather_limited_concurrency(
            (httpx_get(pool, []) for _ in range(REQUESTS)), CONCURRENCY * 2
        )
        ...
        with profile():
            for _ in range(REPEATS):
                await gather_limited_concurrency(
                    (httpx_get(pool, timings) for _ in range(REQUESTS))
                )
        axis.plot(
            [*range(len(timings))], timings, label=f"httpx (tot={duration(start)}ms)"
        )

    connector = aiohttp.TCPConnector(limit=POOL_LIMIT.max_connections)
...
def run_sync_requests(axis: Axes) -> None:
    ...
    def httpx_get(client: httpx.Client, timings: List[int]) -> None:
        start = time.monotonic()
    ...
    transport = httpx.HTTPTransport(limits=POOL_LIMIT)
    with httpx.Client(transport=transport) as pool:
        # warmup
        with ThreadPoolExecutor(max_workers=CONCURRENCY * 2) as exec:
            run_in_executor(
                (lambda: httpx_get(pool, []) for _ in range(REQUESTS)),
                exec,
            )
        ...
        with profile():
            for _ in range(REPEATS):
                run_in_executor(
                    (lambda: httpx_get(pool, timings) for _ in range(REQUESTS)), exec
                )
    ...
    with urllib3.HTTPConnectionPool(
        "localhost", PORT, maxsize=POOL_LIMIT.max_connections
    ) as urllib3_pool:
        # warmup
        with ThreadPoolExecutor(max_workers=CONCURRENCY * 2) as exec:
            run_in_executor(
