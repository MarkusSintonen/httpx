...
__all__ = ["Cookies", "Request", "Response"]
...
def _convert_headers(headers: CIMultiDict[str] | None) -> CIMultiDict[str]:
    def conv(k: str | bytes) -> str:
        return k.decode() if isinstance(k, bytes) else k

    if not headers:
        return CIMultiDict[str]()
    if isinstance(headers, Mapping):
        return CIMultiDict[str]((conv(k).lower(), conv(v)) for k, v in headers.items())
    return CIMultiDict[str]((conv(k).lower(), conv(v)) for k, v in headers)
...
class Request:
    def __init__(
        ...
        *,
        ...
        self.headers = _convert_headers(headers)
...
class Response:
    ...
    def http_version(self) -> str:
        try:
            ...
            return http_version.decode("ascii", errors="ignore") if isinstance(http_version, bytes) else http_version
    ...
    def _get_content_decoder(self) -> ContentDecoder:
        ...
        if not hasattr(self, "_decoder"):
            ...
            values = self.headers.getall("content-encoding", default=[])
            for value in values:
